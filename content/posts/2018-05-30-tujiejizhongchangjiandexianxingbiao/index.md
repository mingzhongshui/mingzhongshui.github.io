---
date: 2018-05-30T21:31:00+00:00
categories: ["后端"]
title: 图解几种常见的线性表
slug: tujiejizhongchangjiandexianxingbiao
#description: 线性表什么是线性表？就是一种连续或间断存储的数组，这里的连续
tags: ["单向链表", "双向链表", "数据结构", "栈", "线性表", "队列"]
featured: false
draft: false
excerpt: 线性表什么是线性表？就是一种连续或间断存储的数组，这里的连续和间断是针对物理内存空间中线性表元素之间是否连续，其中连续数组对应内置数组的实现方式，间断数组对应的是指针的实现方式，这种方式也称为链表实现
---
...
php也可以实现简单的栈，由于php中没有提供很好的操作指针的方式，所以只能通过数组的方式实现。使用连个函数就可以，它们分别是 [array_push][2]和[array_pop][3]
...
同样的，php中也可以以数组的形式实现队列，两个函数[array_push][6]和[array_shift][7] 
