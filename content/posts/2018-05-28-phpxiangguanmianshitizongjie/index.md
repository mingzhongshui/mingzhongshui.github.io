---
date: 2018-05-28T21:46:00+00:00
categories: ["其他", "后端"]
title: PHP相关面试题总结
slug: phpxiangguanmianshitizongjie
#description: 数据库Btree和hash索引的区别HashHash索引，当
tags: ["php", "数据库", "面试"]
featured: false
draft: false
excerpt: 数据库Btree和hash索引的区别HashHash索引，当我们要给某张表某列增加索引时，将这张表的这一列进行哈希算法计算，得到哈希值，排序在哈希数组上。所以Hash索引可以一次定位，其效率很高，而B
---
...
<!--more-->
 ...
 3. 由于哈希值是按照顺序排列的，但是哈希值所对应得真实数据并不一定是按照顺序排列，所以不能进行排序比较;
